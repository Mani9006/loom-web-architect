...
function scoreHeader(data: ResumeJSON): ATSSectionScore {
  ...
  if (!data.header.name.trim()) {
    issues.push({
      ...
      title: "Missing name",
      description:
        "ATS systems require a full name to create a candidate profile. Without it, your resume will be rejected.",
      fix: "Add your full legal name",
    });
  } else {
    score += 3;
    if (data.header.name.toUpperCase() === data.header.name && data.header.name.length > 3) {
      issues.push({
        ...
        section: "Contact Info",
        severity: "warning",
        title: "Name is ALL CAPS",
        description: "Some ATS parsers (Taleo, Workday) misread ALL CAPS names. Use Title Case instead.",
    ...
    if (/[<>{}[\]\\\/|@#$%^&*()+=~`]/.test(data.header.name)) {
      issues.push({
        ...
        description: "Special characters may corrupt your name in ATS databases.",
      });
      score -= 1;
    }
  }
  ...
  if (!data.header.email.trim()) {
    issues.push({
      ...
      description: "Without an email, recruiters cannot contact you. ATS will flag your application as incomplete.",
    });
  } else {
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.header.email)) {
      issues.push({
        ...
        description: "ATS may reject improperly formatted email addresses.",
      });
    } else {
      score += 2;
    }
  ...
  if (!data.header.phone.trim()) {
    issues.push({
      ...
      description: "Most recruiters and ATS expect a phone number for initial screening calls.",
    });
  } else {
    score += 2;
  }
  ...
  if (!data.header.location.trim()) {
    issues.push({
      ...
      description: "ATS location filters will exclude your resume. Add 'City, State' or 'Remote'.",
    });
  } else {
    score += 1;
  }
  ...
  if (!data.header.title.trim()) {
    issues.push({
      ...
      description: "A target job title (e.g., 'Senior Software Engineer') helps ATS match you to relevant positions.",
    });
  } else {
    score += 1;
  }
  ...
  if (!data.header.linkedin.trim()) {
    issues.push({
      ...
      description: "LinkedIn profiles help recruiters verify your background. Many ATS parse LinkedIn data.",
    });
  } else {
    score += 1;
    if (!/linkedin\.com\/in\//i.test(data.header.linkedin)) {
      issues.push({
        ...
        section: "Contact Info",
        severity: "suggestion",
        title: "Non-standard LinkedIn URL",
        description: "Use the standard format: linkedin.com/in/your-name",
      });
    }
  }
...
function scoreSummary(data: ResumeJSON): ATSSectionScore {
  ...
  if (!data.summary.trim()) {
    issues.push({
      ...
      description:
        "A summary is the first thing ATS scans for keyword matching. Without it, you miss critical keyword opportunities and recruiters have no quick overview.",
      fix: "Generate a professional summary",
    });
  ...
  if (words < 20) {
    issues.push({
      ...
      description: `Only ${words} words. ATS keyword matching works best with 30-60 words. Short summaries miss keyword opportunities.`,
      fix: "Expand summary to 30-60 words",
    });
    score += 1;
  } else if (words > 80) {
    issues.push({
      ...
      description: `${words} words. Recruiters spend 6-7 seconds scanning. Keep under 60 words for maximum impact.`,
      fix: "Condense summary to under 60 words",
    });
    score += 3;
  } else {
    score += 4;
  ...
  if (/\b(I|me|my|myself)\b/.test(data.summary)) {
    issues.push({
      ...
      description:
        "Professional summaries should use implied first person. 'I managed a team' → 'Managed a team of 15 engineers'. This is an industry standard.",
      fix: "Remove first-person pronouns",
    });
  } else {
    score += 2;
  }
  ...
  if (METRIC_PATTERN.test(data.summary) || PERCENTAGE_PATTERN.test(data.summary)) {
    ...
    issues.push({
      ...
      section: "Professional Summary",
      severity: "suggestion",
      title: "No quantified results in summary",
      description:
        "Adding metrics like '8+ years experience' or 'managed $2M budget' makes your summary more compelling and ATS-friendly.",
    });
...
function scoreExperience(data: ResumeJSON): ATSSectionScore {
  ...
  if (validExp.length === 0) {
    issues.push({
      ...
      description:
        "Work experience is the most heavily weighted section by all major ATS systems (Taleo, Workday, Greenhouse). Without it, your resume will score extremely low.",
    });
  ...
  if (dateStrings.length > 1) {
    ...
    if (lastIsCurrent && !firstIsCurrent) {
      issues.push({
        ...
        section: "Work Experience",
        severity: "warning",
        title: "Not in reverse chronological order",
        description: "ATS systems and recruiters expect most recent role first. Reorder from newest to oldest.",
      });
    } else {
      score += 2;
    }
  ...
  validExp.forEach((exp, idx) => {
    ...
    if (!exp.role.trim()) {
      issues.push({
        ...
        section: "Work Experience",
        severity: "critical",
        title: `Experience ${idx + 1}: Missing job title`,
        description:
          "ATS uses job titles as PRIMARY matching criteria. Without a title, this role won't match any job listings.",
      });
    } else {
      thisRoleScore += 2;
    }
    ...
    if (!exp.start_date.trim() || !exp.end_date.trim()) {
      issues.push({
        ...
        section: "Work Experience",
        severity: "warning",
        title: `${exp.role || `Experience ${idx + 1}`}: Missing dates`,
        description:
          "ATS calculates total years of experience from dates. Missing dates means your experience won't count toward requirements like '5+ years experience'.",
      });
    } else {
      ...
      if (!startValid || !endValid) {
        issues.push({
          ...
          section: "Work Experience",
          severity: "suggestion",
          title: `${exp.role || `Experience ${idx + 1}`}: Non-standard date format`,
          description: "Use 'Month Year' (e.g., 'January 2023' or 'Jan 2023') for best ATS compatibility.",
        });
        thisRoleScore += 0.5;
      } else {
        thisRoleScore += 1;
    ...
    if (exp.bullets.length === 0) {
      issues.push({
        ...
        section: "Work Experience",
        severity: "critical",
        title: `${exp.role || `Experience ${idx + 1}`}: No bullet points`,
        description:
          "ATS keyword matching relies heavily on bullet point content. Without bullets, this role contributes zero keywords.",
        fix: "Add 3-6 bullet points with achievements",
      });
    } else if (exp.bullets.length < 3) {
      issues.push({
        id: `${prefix}-few-bullets`,
        section: "Work Experience",
        severity: "warning",
        title: `${exp.role || `Experience ${idx + 1}`}: Only ${exp.bullets.length} bullet(s)`,
        description: "Best practice is 3-6 bullets per role. Fewer bullets means fewer keyword matching opportunities.",
        fix: "Add more bullet points",
      });
      thisRoleScore += 1;
    } else if (exp.bullets.length > 8) {
      issues.push({
        id: `${prefix}-many-bullets`,
        section: "Work Experience",
        severity: "suggestion",
        title: `${exp.role || `Experience ${idx + 1}`}: Too many bullets (${exp.bullets.length})`,
        description: "Keep 4-6 bullets per role. Too many bullets dilute impact and make the resume too long.",
      });
      thisRoleScore += 2;
    } else {
      thisRoleScore += 3;
    ...
    if (exp.bullets.length > 0) {
      ...
      if (actionRatio >= 0.8) {
        ...
        issues.push({
          ...
          section: "Work Experience",
          severity: "warning",
          title: `${exp.role || `Experience ${idx + 1}`}: ${weak} bullets lack strong action verbs`,
          description:
            "Start every bullet with a strong action verb (Led, Developed, Optimized, Implemented). ATS parsers weight the first word heavily.",
          fix: "Rewrite bullets to start with strong action verbs",
        });
      } else {
        issues.push({
          id: `${prefix}-verbs`,
          section: "Work Experience",
          severity: "warning",
          title: `${exp.role || `Experience ${idx + 1}`}: Most bullets lack action verbs`,
          description: `Only ${bulletsWithAction.length}/${exp.bullets.length} bullets start with strong action verbs. Use words like 'Led', 'Developed', 'Optimized', 'Increased'.`,
          fix: "Rewrite bullets to start with strong action verbs",
        });
      }
    }
    ...
    if (exp.bullets.length > 0) {
      const bulletsWithMetrics = exp.bullets.filter(
        (b) => METRIC_PATTERN.test(b) || PERCENTAGE_PATTERN.test(b) || DOLLAR_PATTERN.test(b) || NUMBER_PATTERN.test(b),
      );
      if (bulletsWithMetrics.length === 0) {
        issues.push({
          ...
          section: "Work Experience",
          severity: "warning",
          title: `${exp.role || `Experience ${idx + 1}`}: No quantified achievements`,
          description:
            "Resumes with numbers get 40% more interviews. Add metrics: percentages, dollar amounts, team sizes, or project counts.",
          fix: "Add quantifiable metrics to bullets",
        });
      } else if (bulletsWithMetrics.length / exp.bullets.length < 0.4) {
        thisRoleScore += 1;
        issues.push({
          id: `${prefix}-few-metrics`,
          section: "Work Experience",
          severity: "suggestion",
          title: `${exp.role || `Experience ${idx + 1}`}: Only ${bulletsWithMetrics.length}/${exp.bullets.length} bullets have metrics`,
          description: "Aim for at least 50% of bullets to include quantifiable results.",
        });
      } else {
        thisRoleScore += 2;
      }
    ...
    exp.bullets.forEach((b, bIdx) => {
      ...
      if (bWords > 30) {
        issues.push({
          id: `${prefix}-b${bIdx}-long`,
          section: "Work Experience",
          severity: "suggestion",
          title: `${exp.role || `Experience ${idx + 1}`}: Bullet ${bIdx + 1} too long`,
          description: `${bWords} words. ATS may truncate long bullets. Keep under 25 words.`,
          fix: "Shorten bullet to under 25 words",
        });
      }
      if (bWords < 5 && b.trim()) {
        issues.push({
          id: `${prefix}-b${bIdx}-short`,
          section: "Work Experience",
          severity: "suggestion",
          title: `${exp.role || `Experience ${idx + 1}`}: Bullet ${bIdx + 1} too brief`,
          description: `Only ${bWords} words. Expand with specific achievements and context.`,
          fix: "Expand bullet with more detail",
        });
      }
    });
    ...
    if (uniqueBullets.size < lowerBullets.length) {
      issues.push({
        ...
        section: "Work Experience",
        severity: "warning",
        title: `${exp.role || `Experience ${idx + 1}`}: Duplicate bullet points`,
        description: "Duplicate content wastes keyword space. Each bullet should be unique.",
      });
    }
...
function scoreEducation(data: ResumeJSON): ATSSectionScore {
  ...
  if (validEdu.length === 0) {
    issues.push({
      ...
      description:
        "Many ATS systems filter by education level (e.g., 'Bachelor's required'). Missing education can auto-reject your application.",
    });
  ...
  validEdu.forEach((edu, idx) => {
    ...
    if (!edu.degree.trim()) {
      issues.push({
        ...
        section: "Education",
        severity: "warning",
        title: `${edu.institution}: Missing degree type`,
        description:
          "ATS filters require degree level (Bachelor's, Master's, PhD). Without it, you won't pass education filters.",
      });
    } else {
      thisScore += 3;
    }
    ...
    if (!edu.graduation_date.trim()) {
      issues.push({
        ...
        section: "Education",
        severity: "suggestion",
        title: `${edu.institution}: Missing graduation date`,
        description: "Dates help ATS verify degree completion and calculate experience timeline.",
      });
    } else {
      thisScore += 2;
    }
    ...
    if (!edu.field.trim()) {
      issues.push({
        ...
        section: "Education",
        severity: "warning",
        title: `${edu.institution}: Missing field of study`,
        description:
          "ATS may filter by major/field (e.g., 'Computer Science degree required'). Include your field of study.",
      });
    } else {
      thisScore += 2;
    }
...
function scoreSkills(data: ResumeJSON): ATSSectionScore {
  ...
  if (allSkills.length === 0) {
    issues.push({
      ...
      description:
        "The Skills section is where ATS performs PRIMARY keyword matching. Without skills, your resume will fail keyword matching for almost every job listing. This is the #1 reason resumes get rejected by ATS.",
    });
  ...
  if (allSkills.length < 5) {
    issues.push({
      ...
      section: "Skills",
      severity: "warning",
      title: `Only ${allSkills.length} skills listed`,
      description:
        "Most competitive resumes list 10-20 relevant skills. Fewer skills means fewer keyword matches with job descriptions.",
    });
    score += 2;
    ...
    issues.push({
      ...
      section: "Skills",
      severity: "suggestion",
      title: `${allSkills.length} skills — could add more`,
      description:
        "You have a decent start. Adding 5-10 more relevant skills will significantly improve ATS match rates.",
    });
    score += 4;
    ...
    issues.push({
      ...
      section: "Skills",
      severity: "suggestion",
      title: "Good skill count, consider adding more",
      description: `${allSkills.length} skills is solid. 15-20 skills is optimal for maximum ATS keyword coverage.`,
    });
    ...
    issues.push({
      ...
      section: "Skills",
      severity: "suggestion",
      title: `${allSkills.length} skills may be too many`,
      description: "Focus on 15-20 most relevant skills. Too many skills can dilute your profile and look unfocused.",
    });
  }
  ...
  if (categories.length < 2) {
    issues.push({
      ...
      description:
        "Organizing skills into categories (Technical Skills, Tools, Frameworks, Soft Skills) helps ATS parse and categorize your abilities.",
    });
    score += 1;
  ...
  if (longSkills.length > 0) {
    issues.push({
      ...
      section: "Skills",
      severity: "suggestion",
      title: "Some skills are too verbose",
      description: `"${longSkills[0]}" looks like a phrase. Keep skills concise (1-3 words each) for ATS keyword matching.`,
    });
    score += 1;
  } else {
    score += 3;
...
function scoreFormatting(data: ResumeJSON): ATSSectionScore {
  ...
  if (/[\u{1F600}-\u{1F64F}\u{1F300}-\u{1F5FF}\u{1F680}-\u{1F6FF}\u{2600}-\u{26FF}]/u.test(allText)) {
    issues.push({
      ...
      section: "Formatting",
      severity: "critical",
      title: "Emojis detected in resume",
      description:
        "ATS systems (Taleo, Workday, iCIMS) CANNOT parse emojis. They will corrupt your resume data and may cause rejection.",
      fix: "Remove all emojis from resume content",
    });
  } else {
    score += 2;
  }
  ...
  if (/\t/.test(allText)) {
    issues.push({
      ...
      section: "Formatting",
      severity: "warning",
      title: "Tab characters detected",
      description: "Tabs cause ATS parsing errors. Use regular spaces instead.",
    });
  } else {
    score += 1;
  }
  ...
  if (totalWords < 100) {
    issues.push({
      ...
      description: `Only ~${totalWords} words. Competitive resumes have 300-700 words. Your resume lacks enough content for meaningful ATS keyword matching.`,
    });
    ...
    issues.push({
      ...
      section: "Formatting",
      severity: "suggestion",
      title: "Resume could use more content",
      description: `~${totalWords} words. Aim for 400-600 words for a strong single-page resume.`,
    });
    score += 1;
    ...
    issues.push({
      ...
      description: `~${totalWords} words. Unless you have 15+ years experience, aim for 1-2 pages (400-800 words).`,
    });
    score += 2;
  } else {
    score += 3;
  ...
  if (essentialSections < 5) {
    issues.push({
      ...
      section: "Formatting",
      severity: "critical",
      title: `Missing essential sections: ${missingSections.join(", ")}`,
      description: `Your resume is missing ${missingSections.length} essential section(s). ATS systems expect all 5 core sections: Contact Info, Summary, Experience, Education, and Skills.`,
    });
  ...
  if (allDates.length > 2) {
    ...
    if (formatCount > 1) {
      issues.push({
        ...
        description:
          "You're mixing date formats (e.g., 'Jan 2023' and '01/2023'). Use one consistent format throughout for ATS compatibility.",
      });
    } else {
      score += 1;
    }
...
function scoreContentQuality(data: ResumeJSON): ATSSectionScore {
  ...
  if (validProjects.length > 0) {
    ...
    if (hasDescriptions) {
      ...
      issues.push({
        ...
        severity: "suggestion",
        title: "Projects lack descriptions",
        description: "Add 1-3 bullet points per project describing what you built and technologies used.",
      });
    }
    score += projectScore;
  } else {
    ...
    issues.push({
      ...
      description: "Projects demonstrate practical skills, especially valuable for career changers or new graduates.",
    });
  }
  ...
  if (validCerts.length > 0) {
    ...
    if (hasIssuers) {
      ...
      issues.push({
        ...
        section: "Content Quality",
        severity: "suggestion",
        title: "Certifications missing issuing organizations",
        description: "Include the certifying organization (e.g., 'AWS', 'Google', 'PMI') for ATS verification.",
      });
    }
  }
  ...
  if (allBullets.length > 0) {
    const bulletsWithMetrics = allBullets.filter(
      (b) => METRIC_PATTERN.test(b) || PERCENTAGE_PATTERN.test(b) || DOLLAR_PATTERN.test(b) || NUMBER_PATTERN.test(b),
    );
    ...
    if (metricRatio >= 0.5) {
      ...
      issues.push({
        ...
        description: `${bulletsWithMetrics.length}/${allBullets.length} bullets have metrics. Top resumes quantify 50%+ of bullets.`,
      });
      ...
      issues.push({
        ...
        section: "Content Quality",
        severity: "warning",
        title: "Low metrics density",
        description: `Only ${bulletsWithMetrics.length}/${allBullets.length} bullets include numbers. Quantify achievements wherever possible.`,
      });
    } else {
      issues.push({
        id: "cq-no-metrics",
        section: "Content Quality",
        severity: "warning",
        title: "No quantified achievements anywhere",
        description:
          "Zero bullets contain metrics. Add percentages, dollar amounts, team sizes, or counts to demonstrate impact.",
      });
    }
  }
  ...
  if (uniqueWords.size > 150) {
    ...
    issues.push({
      ...
      section: "Content Quality",
      severity: "suggestion",
      title: "Low keyword diversity",
      description:
        "Your resume uses limited vocabulary. Use varied, industry-specific terminology to improve keyword matching.",
    });
  }
...
export function matchJobDescriptionKeywords(data: ResumeJSON, jobDescription: string): KeywordMatch[] {
  ...
  for (let i = 0; i < jdWords.length - 1; i++) {
    ...
    if (
      !stopWords.has(jdWords[i]) &&
      !stopWords.has(jdWords[i + 1]) &&
      jdWords[i].length > 2 &&
      jdWords[i + 1].length > 2
    ) {
...
export function calculateATSScore(data: ResumeJSON, jobDescription?: string): ATSScore {
  ...
  if (overall >= 90) {
    summary =
      "Excellent! Your resume is highly optimized for ATS systems. It should pass most automated screening filters.";
  } else if (overall >= 80) {
    summary =
      "Very good. Your resume is ATS-compatible with minor improvements possible. Focus on the suggestions below to reach 90+.";
  } else if (overall >= 70) {
