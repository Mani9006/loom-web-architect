...
function scoreHeader(data: ResumeJSON): ATSSectionScore {
  ...
  if (!data.header.name.trim()) {
    issues.push({
      ...
      severity: "critical",
      title: "Missing name",
      description: "ATS systems require a full name to create a candidate profile.",
    ...
    if (data.header.name.toUpperCase() === data.header.name && data.header.name.length > 3) {
      issues.push({
        ...
        description: "Some ATS parsers misread ALL CAPS names. Use Title Case.",
    ...
    if (/[<>{}[\]\\\/|@#$%^&*()+=~`]/.test(data.header.name)) {
      issues.push({
        ...
        description: "Special characters in your name may confuse ATS parsers.",
      });
    }
  }
  ...
  if (!data.header.email.trim()) {
    issues.push({
      ...
      severity: "critical",
      title: "Missing email",
      description: "Without an email, recruiters and ATS cannot contact you.",
    });
  } else {
    score += 3;
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.header.email)) {
      issues.push({
        ...
        description: "ATS may reject improperly formatted emails.",
      });
    }
    // Check for unprofessional email domains
    const unprofessionalPatterns = /^[a-z]*\d{4,}@|^(sexy|hot|cool|crazy|funny|gaming|xxx)/i;
    if (unprofessionalPatterns.test(data.header.email)) {
      issues.push({
        id: "h-email-prof",
        section: "Personal Info",
        severity: "suggestion",
        title: "Consider a professional email",
        description: "Use a professional email format like firstname.lastname@domain.com.",
      });
    }
  }
  ...
  if (!data.header.phone.trim()) {
    issues.push({
      ...
      description: "Most recruiters expect a phone number for initial screening.",
    });
  } else {
    score += 2;
  }
  ...
  if (!data.header.location.trim()) {
    issues.push({
      ...
      description: "ATS filters often use location. Add city/state or 'Remote'.",
    });
  } else {
    score += 2;
  }
  ...
  if (!data.header.title.trim()) {
    issues.push({
      ...
      section: "Personal Info",
      severity: "suggestion",
      title: "No job title in header",
      description: "A target title helps ATS match you to relevant roles.",
    });
  } else {
    score += 3;
  }
  ...
  if (!data.header.linkedin.trim()) {
    issues.push({
      ...
      description: "LinkedIn profiles help verify your professional background.",
    });
  } else {
    score += 2;
    ...
    if (!/linkedin\.com\/in\//i.test(data.header.linkedin)) {
      issues.push({
        ...
        section: "Personal Info",
        severity: "suggestion",
        title: "LinkedIn URL format",
        description: "Use the standard LinkedIn profile URL format: linkedin.com/in/your-name",
      });
    }
  }
...
function scoreSummary(data: ResumeJSON): ATSSectionScore {
  ...
  if (!data.summary.trim()) {
    issues.push({
      ...
      description: "A summary helps ATS keyword matching and gives recruiters a quick overview.",
  ...
  if (words < 20) {
    issues.push({
      ...
      description: `Only ${words} words. Aim for 30-60 words for optimal ATS parsing.`,
    ...
    issues.push({
      ...
      description: `${words} words. Keep under 60-80 words to maintain recruiter attention.`,
      fix: "Condense summary to under 80 words",
    });
    score += 8;
  } else {
    score += 10;
  ...
  if (/\b(I|me|my|myself)\b/i.test(data.summary)) {
    issues.push({
      ...
      description: "Professional summaries should avoid 'I', 'me', 'my'. Use implied first person.",
  ...
  if (METRIC_PATTERN.test(data.summary) || PERCENTAGE_PATTERN.test(data.summary)) {
    ...
    issues.push({
      ...
      section: "Summary",
      severity: "suggestion",
      title: "No metrics in summary",
      description: "Adding numbers (e.g., '8+ years', '50% improvement') makes your summary more impactful.",
    });
  }
...
function scoreExperience(data: ResumeJSON): ATSSectionScore {
  ...
  if (validExp.length === 0) {
    issues.push({
      ...
      description: "Work experience is the most heavily weighted section by ATS systems.",
    });
  ...
  // Simple check: if "Present" or "Current" isn't the first entry, flag it
  if (
    dateStrings.length > 1 &&
    /present|current/i.test(dateStrings[dateStrings.length - 1]) &&
    !/present|current/i.test(dateStrings[0])
  ) {
    issues.push({
      id: "e-order",
      section: "Experience",
      ...
      description: "ATS systems and recruiters expect your most recent role first. Reorder from newest to oldest.",
    });
  }
  ...
  validExp.forEach((exp, idx) => {
    ...
    if (!exp.role.trim()) {
      issues.push({
        ...
        description: "ATS uses job titles as primary matching criteria.",
      });
    } else {
      score += 1;
    }
    ...
    if (!exp.start_date.trim() || !exp.end_date.trim()) {
      issues.push({
        ...
        description: "ATS systems calculate experience duration from dates. Missing dates hurt ranking.",
      });
    } else {
      score += 1;
      ...
      if (!startValid || !endValid) {
        issues.push({
          ...
          description: "Use 'Month Year' format (e.g., 'January 2023' or 'Jan 2023') for best ATS compatibility.",
        });
      }
    }
    ...
    if (exp.bullets.length === 0) {
      issues.push({
        ...
        description: "ATS keyword matching relies heavily on bullet point content.",
      ...
      issues.push({
        ...
        description: "Aim for 3-6 bullets per role for optimal keyword coverage.",
    ...
    if (bulletsWithoutAction.length > 0 && exp.bullets.length > 0) {
      ...
      if (ratio > 0.5) {
        issues.push({
          ...
          section: "Experience",
          severity: "warning",
          title: `${exp.role || `Experience ${idx + 1}`}: Weak action verbs`,
          description: `${bulletsWithoutAction.length}/${exp.bullets.length} bullets don't start with strong action verbs. Use words like 'Developed', 'Led', 'Optimized'.`,
    ...
    if (bulletsWithMetrics.length === 0 && exp.bullets.length > 0) {
      issues.push({
        ...
        description:
          "Resumes with numbers get 40% more interviews. Add metrics like percentages, dollar amounts, or team sizes.",
    ...
    exp.bullets.forEach((b, bIdx) => {
      ...
      if (bWords > 30) {
        issues.push({
          ...
          description: `${bWords} words. Keep bullets under 25 words for readability.`,
      ...
      if (bWords < 5) {
        issues.push({
          ...
          description: `Only ${bWords} words. Expand to include specific achievements, metrics, or context.`,
          fix: "Expand bullet with more detail",
        });
      }
    });

    // Check for duplicate/repetitive bullets
    const lowerBullets = exp.bullets.map((b) => b.toLowerCase().trim());
    ...
    if (uniqueBullets.size < lowerBullets.length) {
      issues.push({
        ...
        description: "Remove or rewrite duplicate bullets to maximize keyword variety.",
      });
    }
  });
...
function scoreEducation(data: ResumeJSON): ATSSectionScore {
  ...
  if (validEdu.length === 0) {
    issues.push({
      ...
      description: "Many ATS systems filter by education level. Add your highest degree.",
    });
  ...
  validEdu.forEach((edu, idx) => {
    if (!edu.degree.trim()) {
      issues.push({
        ...
        description: "ATS filters by degree level (Bachelor's, Master's, etc.).",
      });
    } else {
      score += 2;
    }
    if (!edu.graduation_date.trim()) {
      issues.push({
        ...
        description: "Dates help ATS calculate recency of education.",
      });
    } else {
      score += 1;
    }
    if (!edu.field.trim()) {
      issues.push({
        ...
        description: "ATS may filter by field/major for technical roles.",
      });
    } else {
      score += 1;
    }
...
function scoreSkills(data: ResumeJSON): ATSSectionScore {
  ...
  if (allSkills.length === 0) {
    issues.push({
      ...
      description: "Skills section is the #1 ATS keyword matching area. This drastically reduces your match score.",
    });
  ...
  if (allSkills.length < 5) {
    issues.push({
      ...
      description: "Aim for 10-20 relevant skills for optimal ATS matching.",
    });
    score += 3;
    ...
    issues.push({
      ...
      section: "Skills",
      severity: "suggestion",
      title: "Could add more skills",
      description: `${allSkills.length} skills is good, but 15-20 improves ATS match rates.`,
    });
  } else {
    score += 10;
  }
  ...
  if (categories.length < 2) {
    issues.push({
      ...
      section: "Skills",
      severity: "suggestion",
      title: "Only one skill category",
      description: "Organizing skills into categories (Technical, Tools, Soft Skills) improves ATS parsing.",
    });
  } else {
    score += 3;
  }
  ...
  if (longSkills.length > 0) {
    issues.push({
      ...
      description: `"${longSkills[0]}" looks like a phrase, not a skill keyword. Keep skills concise.`,
    });
  } else {
    score += 2;
  }
...
function scoreProjects(data: ResumeJSON): ATSSectionScore {
  ...
  if (validProjects.length === 0) {
    ...
    issues.push({
      ...
      description: "Projects showcase practical skills, especially valuable for career changers or new grads.",
    });
  ...
  validProjects.forEach((proj, idx) => {
    if (proj.bullets.length === 0 || proj.bullets.every((b) => !b.trim())) {
      issues.push({
        ...
        section: "Projects",
        severity: "suggestion",
        title: `${proj.title}: No description`,
        description: "Add 1-3 bullet points describing what you built and the technologies used.",
      });
    } else {
      score += 2;
    }
  ...
  validCerts.forEach((cert, idx) => {
    if (!cert.issuer.trim()) {
      issues.push({
        ...
        severity: "suggestion",
        title: `${cert.name}: Missing issuer`,
        description: "Include the certifying organization for ATS verification.",
      });
    } else {
      score += 1;
    }
...
function scoreFormatting(data: ResumeJSON): ATSSectionScore {
  ...
  if (/[\u{1F600}-\u{1F64F}\u{1F300}-\u{1F5FF}\u{1F680}-\u{1F6FF}\u{2600}-\u{26FF}]/u.test(allText)) {
    issues.push({
      ...
      section: "Formatting",
      severity: "critical",
      title: "Emojis detected in content",
      description: "Most ATS systems cannot parse emojis and may corrupt your resume data.",
  ...
  if (/\t/.test(allText)) {
    issues.push({
      ...
      description: "Tabs can cause parsing errors. Use spaces instead.",
    });
  } else {
    score += 1;
  }
  ...
  if (totalWords < 100) {
    issues.push({
      ...
      description: `Only ~${totalWords} words total. Most competitive resumes have 300-700 words.`,
    });
    ...
    issues.push({
      ...
      description: `~${totalWords} words. For most roles, keep to 1-2 pages (400-700 words).`,
    });
    score += 2;
  } else {
    score += 3;
  ...
  if (sectionCount < 4) {
    issues.push({
      ...
      section: "Formatting",
      severity: "warning",
      title: "Too few resume sections",
      description: `Only ${sectionCount} sections. ATS expects at least: Contact, Summary, Experience, Education, Skills.`,
    });
  } else {
    score += 1;
  }
