...
function getSectionCount(id: SectionId, data: ResumeJSON): number {
  switch (id) {
    ...
    case "experience":
      return data.experience.filter((e) => e.company_or_client).length;
    case "education":
      return data.education.filter((e) => e.institution).length;
    case "skills":
      return Object.values(data.skills).flat().length;
    case "projects":
      return data.projects.filter((p) => p.title).length;
    case "certifications":
      return data.certifications.filter((c) => c.name).length;
    case "languages":
      return (data.languages || []).filter((l) => l.language).length;
    case "volunteer":
      return (data.volunteer || []).filter((v) => v.organization).length;
    case "awards":
      return (data.awards || []).filter((a) => a.title).length;
    default: {
...
export default function ResumeBuilder() {
  ...
  const atsColor =
    atsScore.overall >= 80 ? "text-green-500" : atsScore.overall >= 60 ? "text-yellow-500" : "text-red-500";
    ...
    async (resumeJson: ResumeJSON) => {
      ...
      try {
        const payload = {
          ...
          personal_info: resumeJson.header as any,
          summary: resumeJson.summary,
          experience: resumeJson.experience as any,
          education: resumeJson.education as any,
          skills: resumeJson.skills as any,
          projects: resumeJson.projects as any,
          certifications: resumeJson.certifications as any,
          languages: resumeJson.languages as any,
  ...
  const completenessColor =
    completeness >= 80 ? "text-green-500" : completeness >= 50 ? "text-yellow-500" : "text-red-400";
        ...
        experience:
          parsedData.experience?.length > 0
            ? parsedData.experience.map((e: any) => ({
                ...
                end_date: e.end_date || "",
                location: e.location || "",
                bullets: Array.isArray(e.bullets) ? e.bullets : formatBullets(e.bullets || ""),
              }))
            : data.experience,
        education:
  ...
  const enhanceSummaryWithAI = async () => {
    if (!data.summary && !data.header.title) {
      toast({
        title: "Add content first",
        description: "Write a draft summary or add your job title.",
        variant: "destructive",
      });
      return;
    }
        ...
        [
          {
            role: "system",
            content:
              "You are an expert resume writer. Generate a concise, impactful professional summary (2-4 sentences, under 80 words). Use strong action verbs, quantify achievements. Output ONLY the summary text.",
          },
          {
            role: "user",
  ...
  const enhanceBulletsWithAI = async (expId: string) => {
    ...
    if (!exp || exp.bullets.length === 0) {
      toast({ title: "Add bullets first", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      const result = await streamAIText(
        ...
        [
          {
            role: "system",
            content: `Improve bullet points for ${exp.role} at ${exp.company_or_client}. Start with strong action verbs, add metrics. Return ONLY a JSON array of strings.`,
          },
  ...
  const tailorResumeToJob = async () => {
    if (!jobDescription.trim()) {
      toast({ title: "Add a job description first", variant: "destructive" });
      return;
    }
    setIsTailoring(true);
    try {
      ...
      const result = await streamAIText(
        ...
        [
          {
            role: "system",
            content: `You are an expert resume tailoring assistant. Given a resume JSON and a job description, optimize the resume to better match the job. Improve the summary to target the role, enhance bullet points with relevant keywords, and reorder skills to prioritize job-relevant ones. Return ONLY valid JSON in the EXACT same schema as the input. Do NOT add fabricated experience or skills the person doesn't have - only reword and optimize existing content.`,
          },
          {
            role: "user",
  ...
  const copyAsPlainText = () => {
    ...
    const contact = [data.header.email, data.header.phone, data.header.location, data.header.linkedin]
      .filter(Boolean)
      .join(" | ");
    ...
    if (data.education.some((e) => e.institution)) {
      ...
      data.education
        ...
        .forEach((e) => lines.push(`${e.degree} in ${e.field}, ${e.institution} (${e.graduation_date})`));
    }
    if (Object.values(data.skills).flat().length > 0) {
      lines.push("", "SKILLS");
      Object.entries(data.skills).forEach(([k, v]) => {
        if (v.length > 0) lines.push(`${SKILL_CATEGORY_LABELS[k] || k}: ${v.join(", ")}`);
      });
    }
  ...
  const addEducation = () =>
    setData((prev) => ({
      ...
      education: [
        ...prev.education,
        { id: crypto.randomUUID(), degree: "", field: "", institution: "", gpa: "", graduation_date: "", location: "" },
      ],
    }));
  ...
  const addProject = () =>
    setData((prev) => ({
      ...prev,
      projects: [...prev.projects, { id: crypto.randomUUID(), title: "", organization: "", date: "", bullets: [""] }],
    }));
  ...
  const updateProject = (index: number, updates: Partial<ProjectEntry>) =>
    setData((prev) => ({ ...prev, projects: prev.projects.map((p, i) => (i === index ? { ...p, ...updates } : p)) }));
  const addSkillToCategory = (category: string, skill: string) => {
    ...
    if (!current.includes(skill.trim()))
      setData((prev) => ({ ...prev, skills: { ...prev.skills, [category]: [...current, skill.trim()] } }));
  };
  ...
  const addLanguage = () =>
    setData((prev) => ({
      ...prev,
      languages: [...(prev.languages || []), { id: crypto.randomUUID(), language: "", proficiency: "Professional" }],
    }));
  ...
  const addVolunteer = () =>
    setData((prev) => ({
      ...
      volunteer: [
        ...(prev.volunteer || []),
        { id: crypto.randomUUID(), role: "", organization: "", date: "", bullets: [] },
      ],
    }));
  const removeVolunteer = (id: string) =>
    setData((prev) => ({ ...prev, volunteer: (prev.volunteer || []).filter((v) => v.id !== id) }));
  const updateVolunteer = (id: string, field: keyof VolunteerEntry, value: any) =>
    setData((prev) => ({
      ...prev,
  ...
  const addAward = () =>
    setData((prev) => ({
      ...prev,
      awards: [...(prev.awards || []), { id: crypto.randomUUID(), title: "", issuer: "", date: "" }],
    }));
  ...
  const renameSection = (sectionId: SectionId, newLabel: string) => {
    ...
    if (sectionId.startsWith("custom_")) {
      setData((prev) => ({
        ...prev,
        customSections: (prev.customSections || []).map((s) => (s.id === sectionId ? { ...s, name: newLabel } : s)),
      }));
    }
    ...
    setData((prev) => ({
      ...
      customSections: (prev.customSections || []).map((s) =>
        s.id === sectionId
          ? {
              ...s,
              entries: [...s.entries, { id: crypto.randomUUID(), title: "", subtitle: "", date: "", bullets: [] }],
            }
          : s,
      ),
    }));
  ...
  const handleLinkedInImport = async () => {
    if (!linkedInText.trim()) {
      toast({ title: "Paste your LinkedIn profile content", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      setData({
        ...
        certifications:
          parsedData.certifications?.length > 0
            ...
            : data.certifications,
        skills:
          parsedData.skills && typeof parsedData.skills === "object"
            ? { ...data.skills, ...parsedData.skills }
            : data.skills,
        projects:
          parsedData.projects?.length > 0
            ? parsedData.projects.map((p: any) => ({
        ...
        languages:
          parsedData.languages?.length > 0
            ? parsedData.languages.map((l: any) => ({
                id: crypto.randomUUID(),
                language: l.language || "",
      ...
      toast({
        ...
        description: "Your resume has been populated from your LinkedIn data.",
      });
    } catch (error) {
      toast({
        title: "LinkedIn import failed",
        description: error instanceof Error ? error.message : "Failed to parse LinkedIn data",
        variant: "destructive",
      });
    } finally {
  ...
  const previewData: ResumeJSON = {
    header: hiddenSections.has("personal")
  ...
  return (
    <div className="flex h-full w-full overflow-hidden bg-background">
      ...
      <div
        className={cn(
          "w-full lg:w-[48%] lg:min-w-[380px] lg:max-w-[520px] flex flex-col min-h-0 border-r border-border",
      ...
      >
              ...
              <div className="mt-2 p-3 bg-muted/40 rounded-lg border border-border/30 space-y-3">
                <div className="space-y-1">
                  <p className="text-xs text-muted-foreground">
                    Copy your LinkedIn profile page content (Ctrl+A → Ctrl+C on your LinkedIn profile) and paste it
                    below. AI will extract your experience, education, skills, and more.
                  </p>
                </div>
                <Textarea
                ...
                >
                  {isParsingLinkedIn ? (
                    <>
                      <Loader2 className="h-3.5 w-3.5 animate-spin" /> AI is parsing your LinkedIn profile...
                    </>
                  ) : (
                    <>
                      <Linkedin className="h-3.5 w-3.5" /> Import LinkedIn Profile
                    </>
                  )}
                </Button>
              </div>
        ...
        <div className="px-4 py-3 border-b border-border bg-gradient-to-r from-primary/5 to-transparent">
          ...
          <DocumentUpload
            ...
            label={isParsingResume ? "AI is parsing your resume..." : "Drop any file here - PDF, Word, Image, Text"}
          />
        </div>
            ...
            >
              ...
              {keywordMatches && (
                <Badge variant="outline" className="text-[10px] h-5 border-orange-500/30">
                  {keywordMatches.matched.length}/{keywordMatches.matched.length + keywordMatches.missing.length}{" "}
                  keywords
                </Badge>
              )}
            ...
            {showJobTarget && (
              <div className="mt-2 p-3 bg-muted/40 rounded-lg border border-border/30 space-y-3">
                          ...
                          <span
                            className={cn(
                              ...
                              keywordMatches.matched.length /
                                (keywordMatches.matched.length + keywordMatches.missing.length) >=
                                0.7
                                ? "text-green-500"
                                : keywordMatches.matched.length /
                                      (keywordMatches.matched.length + keywordMatches.missing.length) >=
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          {keywordMatches.missing.slice(0, 20).map((kw) => (
                            <Badge key={kw} variant="outline" className="text-[10px] border-red-500/30 text-red-500">
                              {kw}
                            </Badge>
                          ))}
                          {keywordMatches.missing.length > 20 && (
                            <span className="text-[10px] text-muted-foreground">
                              +{keywordMatches.missing.length - 20} more
                            </span>
                          )}
                        </div>
                      </div>
                    ...
                    {keywordMatches.matched.length > 0 && (
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          ...
                          {keywordMatches.matched.length > 15 && (
                            <span className="text-[10px] text-muted-foreground">
                              +{keywordMatches.matched.length - 15} more
                            </span>
                          )}
                        </div>
                      </div>
                ...
                >
                  {isTailoring ? (
                    <>
                      <Loader2 className="h-3.5 w-3.5 animate-spin" /> AI is tailoring your resume...
                    </>
                  ) : (
                    <>
                      <Zap className="h-3.5 w-3.5" /> Tailor Resume to This Job
                    </>
                  )}
                </Button>
              </div>
          ...
          <div className={isParsingResume ? "opacity-30 pointer-events-none" : ""}>
            <Accordion type="multiple" defaultValue={["personal", "summary"]} className="px-3 py-1">
              {activeSections.map((section) => {
                ...
                const sectionIcon = section.builtIn ? (
                  BUILT_IN_ICONS[section.id as BuiltInSectionId]
                ) : (
                    ...
                    <div className="flex items-center group">
                        ...
                        <span className="flex items-center gap-2 flex-1 min-w-0">
                          ...
                          {editingSectionId === section.id ? (
                            <input
                              ...
                              onBlur={() => {
                                if (editingSectionLabel.trim()) renameSection(section.id, editingSectionLabel.trim());
                                else setEditingSectionId(null);
                              }}
                              onKeyDown={(e) => {
                                if (e.key === "Enter") {
                                  e.preventDefault();
                      ...
                      <div className="flex items-center shrink-0">
                        ...
                        <Tooltip>
                          ...
                          <TooltipContent>{isHidden ? "Show on resume" : "Hide from resume"}</TooltipContent>
                        </Tooltip>
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <button
                                ...
                                className="p-1 mr-1 rounded hover:bg-destructive/10 text-muted-foreground hover:text-destructive"
                              >
                        ...
                        <div className="grid grid-cols-3 gap-x-2 gap-y-1.5">
                          {(
                            [
                              ...
                              { field: "title" as const, label: "Title", placeholder: "Data Engineer" },
                              {
                              ...
                              { field: "phone" as const, label: "Phone", placeholder: "+1 555-123-4567" },
                              { field: "location" as const, label: "Location", placeholder: "Dallas, TX" },
                              { field: "linkedin" as const, label: "LinkedIn", placeholder: "linkedin.com/in/..." },
                            ] as {
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <Button
                              ...
                              disabled={aiEnhancingSection === "summary"}
                              className="h-6 text-[10px] gap-1 px-2"
                            >
                              {aiEnhancingSection === "summary" ? (
                                <>
                                  <Loader2 className="h-3 w-3 animate-spin" /> Enhancing...
                                </>
                              ) : (
                                <>
                                  <Sparkles className="h-3 w-3" /> {data.summary ? "Enhance" : "Generate"}
                                </>
                              )}
                            </Button>
                          </div>
                      ...
                      {section.id === "experience" && (
                        <div className="space-y-1">
                          <Reorder.Group
                            ...
                            onReorder={(newOrder) => setData((prev) => ({ ...prev, experience: newOrder }))}
                            className="space-y-1"
                          >
                            {data.experience.map((exp, index) => (
                              <Reorder.Item
                                key={exp.id}
                                value={exp}
                                className="rounded border border-border/40 list-none"
                              >
                                {/* Summary row - always visible */}
                                <div
                                  className={cn(
                                    "flex items-center gap-1 px-1 py-1.5 cursor-pointer hover:bg-muted/50 transition-colors",
                                    expandedExpId === exp.id && "bg-muted/40",
                                  )}
                                ...
                                >
                                  ...
                                  <span className="text-xs font-medium truncate flex-1 min-w-0">
                                    {exp.role || exp.company_or_client
                                      ? `${exp.role || "Role"}${exp.company_or_client ? ` @ ${exp.company_or_client}` : ""}`
                                      : `Experience ${index + 1}`}
                                  </span>
                                  {exp.start_date && (
                                  ...
                                  <div className="flex items-center shrink-0" onClick={(e) => e.stopPropagation()}>
                                    {exp.bullets.length > 0 && (
                                      <button
                                        type="button"
                                        onClick={() => enhanceBulletsWithAI(exp.id)}
                                        disabled={aiEnhancingSection === `exp-${exp.id}`}
                                        className="p-0.5 rounded hover:bg-muted text-primary"
                                      >
                                        {aiEnhancingSection === `exp-${exp.id}` ? (
                          ...
                          <button
                            ...
                            className="w-full flex items-center justify-center gap-1.5 py-1.5 text-xs text-muted-foreground hover:text-foreground border border-dashed border-border/50 rounded hover:bg-muted/30 transition-colors"
                          >
                      ...
                      {section.id === "education" && (
                        <div className="space-y-1">
                          <Reorder.Group
                            ...
                            onReorder={(newOrder) => setData((prev) => ({ ...prev, education: newOrder }))}
                            className="space-y-1"
                          >
                            {data.education.map((edu, eduIndex) => (
                              ...
                              >
                                <div
                                  className={cn(
                                    "flex items-center gap-1 px-1 py-1.5 cursor-pointer hover:bg-muted/50 transition-colors",
                                    expandedEduId === edu.id && "bg-muted/40",
                                  )}
                                ...
                                >
                                  ...
                                  <ChevronDown
                                    className={cn(
                                      "h-3 w-3 text-muted-foreground shrink-0 transition-transform",
                                      expandedEduId === edu.id && "rotate-180",
                                    )}
                                  />
                                  <span className="text-xs font-medium truncate flex-1 min-w-0">
                        ...
                        <div className="space-y-2">
                            ...
                            <div key={categoryKey} className="space-y-1">
                              <div className="flex items-center gap-1">
                                <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground flex-1">
                                  {SKILL_CATEGORY_LABELS[categoryKey] ||
                              ...
                              <div className="flex flex-wrap gap-1">
                                {skills.map((skill) => (
                                  <Badge key={skill} variant="secondary" className="text-[10px] h-5 gap-0.5 px-1.5">
                                    {skill}
                                    <button
                                      type="button"
                                      onClick={() => removeSkillFromCategory(categoryKey, skill)}
                                ...
                                <Input
                                  ...
                                  className="bg-background h-5 text-[10px] w-20 min-w-0 px-1.5 border-dashed"
                                />
                              </div>
                            </div>
                          ))}
                          <div className="flex gap-1.5 items-center">
                            <Input
                              ...
                              className="bg-background h-6 text-[10px] flex-1"
                              onKeyDown={(e) => {
                                if (e.key === "Enter" && newCategoryName.trim()) {
                                  const key = newCategoryName
                                    ...
                                    .replace(/\s+/g, "_")
                                    .replace(/&/g, "");
                                  if (!data.skills[key]) setData((p) => ({ ...p, skills: { ...p.skills, [key]: [] } }));
                                  setNewCategoryName("");
                                }
                              }}
                            />
                          ...
                          {Object.keys(data.skills).length === 0 && (
                            <p className="text-[10px] text-muted-foreground text-center py-1">
                              No skills yet. Upload a resume or add above.
                            </p>
                          )}
                        </div>
                      )}
                      ...
                      {section.id === "projects" && (
                        <div className="space-y-1">
                          <Reorder.Group
                            ...
                            onReorder={(newOrder) => setData((prev) => ({ ...prev, projects: newOrder }))}
                            className="space-y-1"
                          >
                            {data.projects.map((project, index) => (
                              ...
                              >
                                <div
                                  className={cn(
                                    "flex items-center gap-1 px-1 py-1.5 cursor-pointer hover:bg-muted/50 transition-colors",
                                    expandedProjIdx === index && "bg-muted/40",
                                  )}
                                ...
                                >
                                  ...
                                  <ChevronDown
                                    className={cn(
                                      "h-3 w-3 text-muted-foreground shrink-0 transition-transform",
                                      expandedProjIdx === index && "rotate-180",
                                    )}
                                  />
                                  <span className="text-xs font-medium truncate flex-1 min-w-0">
                        ...
                        <div className="space-y-1">
                          {data.certifications.map((cert) => (
                            <div key={cert.id} className="rounded border border-border/40">
                              <div
                                className={cn(
                                  "flex items-center gap-1.5 px-2 py-1.5 cursor-pointer hover:bg-muted/50 transition-colors",
                                  expandedCertId === cert.id && "bg-muted/40",
                                )}
                              ...
                              >
                                ...
                                {cert.date && (
                                  <span className="text-[10px] text-muted-foreground shrink-0">{cert.date}</span>
                                )}
                                <div className="flex items-center shrink-0" onClick={(e) => e.stopPropagation()}>
                                  <button
                                    type="button"
                                ...
                                <div className="px-2 pb-2 pt-1 space-y-1.5 border-t border-border/30 bg-muted/20">
                                  <div className="grid grid-cols-3 gap-1.5">
                                    <Input
                                      ...
                                      className="bg-background h-7 text-xs col-span-2"
                                    />
                                    <Input
                                      placeholder="Date"
                                      value={cert.date}
                      ...
                      {section.id === "languages" && (
                        <div className="space-y-1">
                          {(data.languages || []).map((lang) => (
                            <div key={lang.id} className="rounded border border-border/40">
                              <div
                                className={cn(
                                  "flex items-center gap-1.5 px-2 py-1.5 cursor-pointer hover:bg-muted/50 transition-colors",
                                  expandedLangId === lang.id && "bg-muted/40",
                                )}
                              ...
                              >
                                <ChevronDown
                                  className={cn(
                                    "h-3 w-3 text-muted-foreground shrink-0 transition-transform",
                                    expandedLangId === lang.id && "rotate-180",
                                  )}
                                />
                                <span className="text-xs font-medium truncate flex-1">
                          ...
                          {(data.languages || []).length === 0 && (
                            <p className="text-[10px] text-muted-foreground text-center py-1">No languages added yet</p>
                          )}
                          <button
                            type="button"
                            onClick={addLanguage}
                      ...
                      {section.id === "volunteer" && (
                        <div className="space-y-1">
                          {(data.volunteer || []).map((vol) => (
                            <div key={vol.id} className="rounded border border-border/40">
                              <div
                                className={cn(
                                  "flex items-center gap-1.5 px-2 py-1.5 cursor-pointer hover:bg-muted/50 transition-colors",
                                  expandedVolId === vol.id && "bg-muted/40",
                                )}
                              ...
                              >
                                <ChevronDown
                                  className={cn(
                                    "h-3 w-3 text-muted-foreground shrink-0 transition-transform",
                                    expandedVolId === vol.id && "rotate-180",
                                  )}
                                />
                                <span className="text-xs font-medium truncate flex-1">
                                  {vol.role || vol.organization
                                    ? `${vol.role || "Role"}${vol.organization ? ` @ ${vol.organization}` : ""}`
                                    : "New Volunteer Experience"}
                                </span>
                                {vol.date && (
                                  <span className="text-[10px] text-muted-foreground shrink-0">{vol.date}</span>
                                )}
                                <div className="flex items-center shrink-0" onClick={(e) => e.stopPropagation()}>
                                  <button
                                    type="button"
                              ...
                              {expandedVolId === vol.id && (
                                <div className="px-2 pb-2 pt-1 space-y-1.5 border-t border-border/30 bg-muted/20">
                                  ...
                                  />
                                  <Textarea
                                    placeholder={"Key contributions (one per line)"}
                          ...
                          <button
                            ...
                            className="w-full flex items-center justify-center gap-1.5 py-1.5 text-xs text-muted-foreground hover:text-foreground border border-dashed border-border/50 rounded hover:bg-muted/30 transition-colors"
                          >
                            <Plus className="h-3 w-3" /> Add Volunteer Experience
                          </button>
                        </div>
                      )}
                      ...
                      {section.id === "awards" && (
                        <div className="space-y-1">
                          {(data.awards || []).map((award) => (
                            <div key={award.id} className="rounded border border-border/40">
                              <div
                                className={cn(
                                  "flex items-center gap-1.5 px-2 py-1.5 cursor-pointer hover:bg-muted/50 transition-colors",
                                  expandedAwardId === award.id && "bg-muted/40",
                                )}
                              ...
                              >
                                <ChevronDown
                                  className={cn(
                                    "h-3 w-3 text-muted-foreground shrink-0 transition-transform",
                                    expandedAwardId === award.id && "rotate-180",
                                  )}
                                />
                                <span className="text-xs font-medium truncate flex-1">
                                ...
                                {award.date && (
                                  <span className="text-[10px] text-muted-foreground shrink-0">{award.date}</span>
                                )}
                                <div className="flex items-center shrink-0" onClick={(e) => e.stopPropagation()}>
                                  <button
                                    type="button"
                              ...
                              {expandedAwardId === award.id && (
                                <div className="px-2 pb-2 pt-1 space-y-1.5 border-t border-border/30 bg-muted/20">
                                  <Input
                                    placeholder="Award/Publication Title"
                                    value={award.title}
                          ...
                          <button
                            ...
                            className="w-full flex items-center justify-center gap-1.5 py-1.5 text-xs text-muted-foreground hover:text-foreground border border-dashed border-border/50 rounded hover:bg-muted/30 transition-colors"
                          >
                            <Plus className="h-3 w-3" /> Add Award/Publication
                          </button>
                        </div>
                      )}
                              ...
                              {cs.entries.map((entry) => (
                                <div key={entry.id} className="rounded border border-border/40">
                                  <div
                                    className={cn(
                                      "flex items-center gap-1.5 px-2 py-1.5 cursor-pointer hover:bg-muted/50 transition-colors",
                                      expandedCustomId === entry.id && "bg-muted/40",
                                    )}
                                  ...
                                  >
                                    <ChevronDown
                                      className={cn(
                                        "h-3 w-3 text-muted-foreground shrink-0 transition-transform",
                                        expandedCustomId === entry.id && "rotate-180",
                                      )}
                                    />
                                    <span className="text-xs font-medium truncate flex-1">
                                    ...
                                    <div className="px-2 pb-2 pt-1 space-y-1.5 border-t border-border/30 bg-muted/20">
                                      ...
                                      <div className="grid grid-cols-2 gap-1.5">
                                        ...
                                        <Input
                                          ...
                                          value={entry.date}
                                          onChange={(e) =>
                                            updateCustomEntry(section.id, entry.id, "date", e.target.value)
                                          }
                                          className="bg-background h-7 text-xs"
                                        />
                                      </div>
                          ...
                          <div className="mt-2 p-2 rounded-lg bg-amber-500/5 border border-amber-500/20 space-y-1.5">
                            <div className="flex items-center justify-between">
                              ...
                              {(atsName === "Summary" || atsName === "Experience" || atsName === "Skills") && (
                                ...
                                >
                                  {aiFixingSection === atsName ? (
                                    <>
                                      <Loader2 className="h-2.5 w-2.5 animate-spin" /> Fixing...
                                    </>
                                  ) : (
                                    <>
                                      <Wand2 className="h-2.5 w-2.5" /> AI Fix All
                                    </>
                                  )}
                                </Button>
                              )}
                      ...
                      <Button
                        ...
                        onClick={(e) => {
                          ...
                          if (input && input.value.trim()) {
                            addCustomSection(input.value.trim());
                            input.value = "";
                          }
                        }}
                      >
      ...
      <div className={cn("flex-1 flex flex-col min-w-0 bg-muted/20", mobileView !== "preview" && "hidden lg:flex")}>
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border bg-card">
            ...
            >
              ...
              {atsScore.issues.length > 0 && (
                <span className="text-[10px] font-normal opacity-70">
                  ({atsScore.issues.filter((i) => i.severity === "critical").length} critical)
                </span>
              )}
            </button>
          </div>
          <div className="flex items-center gap-1.5">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  className="h-8 gap-1.5"
                >
                  <FileText className="h-3.5 w-3.5" /> <span className="hidden sm:inline">Word</span>
                </Button>
              </TooltipTrigger>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="outline" size="sm" onClick={copyAsPlainText} className="h-8 gap-1.5">
                  {copiedField === "resume" ? (
                    <Check className="h-3.5 w-3.5 text-green-500" />
                  ) : (
                    <Copy className="h-3.5 w-3.5" />
                  )}{" "}
                  <span className="hidden sm:inline">{copiedField === "resume" ? "Copied!" : "Copy"}</span>
                </Button>
              </TooltipTrigger>
              <TooltipContent>Copy as plain text (for pasting into forms)</TooltipContent>
            </Tooltip>
            <Button size="sm" onClick={handleDownloadPDF} disabled={isExporting} className="h-8 gap-1.5">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-semibold">
                    {atsScore.passesATS ? "ATS Compatible" : "Needs Improvement"}
                  </div>
                  <div className="text-[10px] text-muted-foreground">
                    {atsScore.issues.length} issues found (
                    {atsScore.issues.filter((i) => i.severity === "critical").length} critical,{" "}
                    {atsScore.issues.filter((i) => i.severity === "warning").length} warnings)
                  </div>
                </div>
              </div>
              <button type="button" onClick={() => setShowATSDetails(false)} className="p-1 rounded hover:bg-muted">
                <X className="h-4 w-4" />
              </button>
            </div>
              ...
              {atsScore.sections.map((s) => (
                <div key={s.section} className="text-center p-2 rounded-lg bg-muted/40">
                  <div
                    className={cn(
                      "text-lg font-bold",
                      s.score / s.maxScore >= 0.7
                        ? "text-green-500"
                        : s.score / s.maxScore >= 0.4
                          ? "text-yellow-500"
                          : "text-red-500",
                    )}
                  >
